...
namespace FUTBOLERO.Server.Controllers
{
    ...
    public class ColegioController : Controller
    {
        ...
        [HttpGet]
        [Route("api/Colegio/RegresaIdColegio/{idarbitrocolegio}")]
        public int RegresaIdColegio(int idarbitrocolegio)
        {
            int rpta = 0;
            ...
            try
            {
                using (var baseDatos = new FUTBOLEANDOContext())
                {
                    rpta = (int)baseDatos.Arbitrocolegio.Where(p => p.Idarbitrocolegio == idarbitrocolegio && p.Habilitado == 1).First().Idcolegioarbitro;
