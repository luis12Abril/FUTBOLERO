...
namespace FUTBOLERO.Server.Controllers
{
    ...
    public class UsuarioController : Controller
    {
        ...
        public int LoginIidUsuario([FromBody] UsuarioCLS oUsuarioCLS)
        {
            ...
            try
            {
                using (var baseDatos = new FUTBOLEANDOContext())
                {
                    ...
                    //int nveces2 = baseDatos.Usuario.Where(p => p.Nombre.Trim() == oUsuarioCLS.nombre.Trim() && p.Habilitado == 1).Count();
                    //int nveces3 = baseDatos.Usuario.Where(p => p.Contraseña == dataCifradaCadena && p.Habilitado == 1).Count();

                    int nveces = baseDatos.Usuario.Where(p => p.Nombre.Trim() == oUsuarioCLS.nombre.Trim() && p.Contraseña == dataCifradaCadena && p.Habilitado == 1).Count();
                    if (nveces == 0)
                    {
                        rpta = 0;
                    }
                    ...
                    {
                        rpta = baseDatos.Usuario.Where(p => p.Nombre.Trim() == oUsuarioCLS.nombre.Trim() && p.Contraseña == dataCifradaCadena && p.Habilitado == 1).First().Idusuario;
                    }
                }
            }
            catch (Exception ex)
            ...
            try
            {
                using (var baseDatos = new FUTBOLEANDOContext())
                {
                    ...
                    else
                    {
                        ...
                        oUsuario.Nombre = oNuevoUsuarioCLS.nombreusuario.Trim();
                        oUsuario.Visitas = 1;
                        oUsuario.Visitascel = 0;
                        oUsuario.Idarbitrocolegio = 0;

                        oUsuario.Fechaalta = DateTime.Now;
                        oUsuario.Origenalta = "WE2";
            ...
            using (var baseDatos = new FUTBOLEANDOContext())
            {
                oUsuarioCLS = (from usuario in baseDatos.Usuario
                               ...
                               select new UsuarioCLS
                               {
                                   ...
                                   contraseña = "12345678*",
                                   idtipousuario = usuario.Idtipousuario.ToString(),
                                   idarbitrocolegio = usuario.Idarbitrocolegio.ToString()
                               }).First();
        ...
        [HttpGet]
        [Route("api/Usuario/RegresaUsuario1/{idusuario}")]
        public string RegresaUsuario1(int idusuario)
        {
            string rpta = "";
        ...
        [HttpGet]
        [Route("api/Usuario/RecEA/{idusuario}")]
